...
export async function runPipeline(input: PipelineInput): Promise<PipelineOutput> {
  ...
  if (chatter) {
    ...
    headBlock.push(
      ...
      "```",
      "",
      `Quy tắc tag (mentionTelegramUserIds):`,
      `- "Nhắc tôi" → mentionTelegramUserIds = ["${chatter.telegramUserId}"] (tag user hiện tại).`,
      `- "Nhắc anh @ha_ntv" → lookup_telegram_user → mentionTelegramUserIds = ["<id ha_ntv>"].`,
      `- "Nhắc cả nhóm" → bỏ mentionTelegramUserIds (không tag ai cụ thể, hoặc tag mình + admin).`,
      "",
