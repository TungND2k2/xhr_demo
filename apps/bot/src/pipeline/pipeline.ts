...
export async function runPipeline(input: PipelineInput): Promise<PipelineOutput> {
  ...
  if (chatter) {
    ...
    headBlock.push(
      ...
      `→ Reminder phải bắn vào CHÍNH chat này (chatId=${chatter.chatId}), KHÔNG DM riêng. Gọi tool ngay:`,
      "```",
      `create_reminder({`,
      `  recipientType: "chat",`,
      ...
      "```",
      "",
      `Tương tự (luôn dùng recipientType="chat" + recipientChatId="${chatter.chatId}"):`,
      `- "5p nữa nhắc tôi gọi A" → dueAt = nowVN+5min`,
      `- "9h sáng mai nhắc tôi B" → dueAt = (mai 09:00 +07:00)`,
      `- "thứ 6 14h nhắc cả nhóm họp" → dueAt = thứ 6 14:00 +07:00 (cùng chat, mọi người sẽ thấy)`,
      "",
